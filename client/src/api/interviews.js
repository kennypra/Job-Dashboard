...
function withJsonErrorHandling(promise) {
  // Network failures (server down, offline) reject before a Response ever
  // exists — normalize those into the same Error shape so callers only
  // have one failure path to handle, per the "never fail silently" rule.
